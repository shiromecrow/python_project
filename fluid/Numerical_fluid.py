...
t=0
Xmax=1.0#xの# 最大値
f=[[0 for i in range((int)(T/dt+T2/dt+1))] for j in range((int)(Xmax/dx+1))]#濃度の位置，時間での2次配列
...
while t < (T/dt):
    X=1
    while X <=(Xmax/dx-1):
        f[X][t+1]=f[X][t]+nu*dt/dx/dx*(f[X+1][t]-2*f[X][t]+f[X-1][t])
        #拡散方程式
...
while t<(T/dt+T2/dt):
    ...
    sigmaX=0
    while X < (Xmax / dx):
        f[X][t + 1] = f[X][t] + nu * dt / dx / dx * (f[X + 1][t] - 2 * f[X][t] + f[X - 1][t])-u*dt/dx*(f[X][t]-f[X-1][t])
        # 移流拡散方程式
        sigmaX = sigmaX + f[X][t + 1]#収束条件の判定
        X = X + 1

    f[0][t + 1] = f[1][t + 1]-dx*u/nu*f[1][t + 1]
    f[(int)(Xmax / dx)][t + 1] = f[(int)(Xmax / dx) - 1][t + 1]-dx*u/nu*f[(int)(Xmax / dx-1)][t + 1]
    t = t + 1
